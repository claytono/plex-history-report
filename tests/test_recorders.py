...
def make_dummy_item():
    class DummyItem:
        ...
        rating = 8.5
        viewOffset = 60000  # milliseconds
        isWatched = True
        viewedAt = datetime(2025, 5, 2, 12, 0, 0)
        username = "user1"
...
def test_serialize_basic_item():
    ...
    # Test episode-specific serialization
    ep = make_dummy_item()
    ep.type = "episode"
    ep.seasonNumber = 1
    ep.index = 2
    result = recorder._serialize_plex_item(ep)
    assert result["seasonNumber"] == 1
    assert result["index"] == 2
    assert result.get("showTitle") == "Show Name"
...
def test_record_raw_and_test_data(tmp_path):
    ...
    assert isinstance(test_content["all_shows"], list)
...
def test_random_anonymized_recent_movies(tmp_path, monkeypatch):
    ...
    for idx, movie in enumerate(recents, start=1):
        assert movie["title"] == f"Recent Movie {idx}"
        assert movie["year"] == 2010
        assert "viewed_at" in movie
